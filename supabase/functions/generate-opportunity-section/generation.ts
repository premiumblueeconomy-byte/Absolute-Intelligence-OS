...
async function validatedCall<T>(schema: z.ZodType<T>, call: ModelCall, role: string, system: string, input: string, maxTokens: number): Promise<T> {
 ...
 try {
  ...
  const issues = error instanceof z.ZodError
   ...
   : [{path:'',message:'Return a complete, valid JSON object. The previous output was invalid or truncated. Keep every string under 200 characters and arrays to the minimum required length.'}];
...
export async function generateSection(section: Section, context: Record<string, unknown>, instructions: string, call: ModelCall) {
 ...
 ].map(async ([role,task])=>({role,result:await validatedCall(briefSchema,call,role,`${RULES} ${task} Focus on the requested section. Use {"summary":"...","findings":[],"candidates":[],"risks":[],"evidence_gaps":[]}; arrays contain at most three strings. Each string must be under 150 characters.`,request,1600)})));
 const review = await validatedCall(reviewSchema,call,'critic',`${RULES} Challenge the specialists. Use {"disagreements":[],"rejected_ideas":[],"required_corrections":[],"validation_priorities":[]}, up to three strings each, under 150 characters per string.`,JSON.stringify({request,briefs}),1600);
